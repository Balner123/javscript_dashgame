...
/*******************************************************************************************************/
/* Vykresluje stavový řádek s průběžnými informacemi o stavu hry */
function statusBar() {
  ...
  fill(color(200));
  /* Výpis počtu zasažených asteroidů */
  text(`Hits: ${hits}`, 100, height - 15);
  /* Výpis úrovně poškození kosmické lodi */
  text(`Spaceship damage: ${round(damage) < 100 ? round(damage) : 100} %`, 250, height - 15);
  /* Procentuální vyjádření stojících (nezasažených) budov */
  text(`Buildings: ${round(buildings.length/50 * 100)} %`, 600, height - 15);
...
function draw() {
  ...
  /* Kontroluje poškození lodi a procento zbylých budov */
  /* Když je loď poškozena nad hodnotu 99 nebo zbylo méně než 30 % budov, bude hra ukončena */
  if (round(damage) > 99 || round(buildings.length/50 * 100) < 30) {
    /* Ukončení smyčky hry */
    noLoop();
